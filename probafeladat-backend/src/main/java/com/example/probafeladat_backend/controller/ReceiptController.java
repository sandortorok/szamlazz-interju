...
import com.example.probafeladat_backend.services.SzamlazzService;
...
public class ReceiptController {
    ...
    private final SzamlazzService szamlazzService;
    private final JAXBContext jaxbContext;

    public ReceiptController(SzamlazzService szamlazzService) throws JAXBException {
    ...
    public ResponseEntity<?> createReceipt(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                ...
                required = true,
                content = @Content(schema = @Schema(implementation = ReceiptRequest.class))
            )
            @Valid @RequestBody ReceiptRequest request) {
        try {
            // ReceiptRequest objektum XML-lé alakítása
            ...
            ReceiptResponse response = szamlazzService.createReceipt(xmlContent);
            
            // Ha a Számlázz.hu API hibát küld, akkor 400 Bad Request-tel visszaküldjük a hibát
    ...
    @Operation(
        ...
        description = "Teszt nyugta létrehozása a resources/samples/receipt-sample.xml minta alapján"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<ReceiptResponse> testReceipt() throws IOException, JAXBException {
        ReceiptResponse response = szamlazzService.testWithSample();
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<List<ReceiptSummaryDto>> getAllReceipts() {
        List<ReceiptSummaryDto> receipts = szamlazzService.getAllReceipts();
    ...
    public ResponseEntity<?> getReceiptById(
            @Parameter(description = "Nyugta azonosítója", required = true)
            @PathVariable Long id) {
        return szamlazzService.getReceiptById(id)
